-- Add developer access level above admin. Admin retains categories/users powers;
-- developer inherits admin and unlocks power tools (templates, queue, map).

alter type public.access_level add value if not exists 'developer' after 'admin';

comment on column public.users.spaces_access_level is
  'Seton Spaces: developer, admin, manager, trusted user, user, guest';

comment on column public.users.help_access_level is
  'Seton Help: developer, admin, manager, worker, trusted user, user, guest';
...
-- Expand account listing for admin user management (trust / ban / restore).
